...
const COLORS = {
  primaryBg: '#1B1D23',
  secondaryBg: '#23262F',
  accent: '#E60000', // High-impact Red
  hover: '#FF3333',   // Hover Red
  textPrimary: '#F2F2F2',
        ...
        >
          ...
          <div className="p-10">
            {submitted ? (
              ...
              >
                <div className="w-20 h-20 bg-[#E60000] rounded-full flex items-center justify-center mx-auto mb-8">
              ...
              <>
                <div className="mb-10 text-center">
                  <span className="text-[#E60000] font-bold text-xs tracking-[0.4em] uppercase mb-3 block">
                  ...
                  <h2 className="text-5xl font-display text-white uppercase tracking-tight leading-none">
                    ...
                    <span className="text-[#E60000]">{type === 'join' ? 'EVOLUTION' : 'FACILITY TOUR'}</span>
                  </h2>
                </div>

                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="relative group">
                    <User className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-[#E60000] transition-colors" size={18} />
                    <input 
                      required
                      type="text" 
                      placeholder="FULL NAME" 
                      className="w-full bg-[#23262F] border border-white/5 rounded-lg pl-12 pr-6 py-5 text-white font-bold text-xs uppercase tracking-widest outline-none focus:border-[#E60000] transition-all"
                    />
                  </div>
                  <div className="relative group">
                    <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-[#E60000] transition-colors" size={18} />
                    <input 
                      required
                      type="email" 
                  ...
                  <div className="relative group">
                    <Phone className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-[#E60000] transition-colors" size={18} />
                    <input 
                      required
                      type="tel" 
                  ...
                  {type === 'join' ? (
                    <div className="space-y-3">
                      ...
                      <select className="w-full bg-[#23262F] border border-white/5 rounded-lg px-6 py-5 text-white font-bold text-xs uppercase tracking-widest outline-none focus:border-[#E60000] transition-all appearance-none">
                    ...
                    <div className="relative group">
                      <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-[#E60000] transition-colors" size={18} />
                      <input 
                        required
                        type="date" 
                        className="w-full bg-[#23262F] border border-white/5 rounded-lg pl-12 pr-6 py-5 text-white font-bold text-xs uppercase tracking-widest outline-none focus:border-[#E60000] transition-all"
                  ...
                  <button 
                    type="submit"
                    className="w-full bg-[#E60000] hover:bg-[#FF3333] py-6 rounded-lg text-white font-display text-lg uppercase tracking-[0.1em] transition-all shadow-xl shadow-red-900/30 flex items-center justify-center gap-3 active:scale-95"
                  >
...
const Navbar = ({ currentPage, setPage, openModal }: { currentPage: string, setPage: (page: string) => void, openModal: (type: 'join' | 'tour') => void }) => {
  ...
  return (
    <div className="fixed w-full z-50 flex justify-center pt-6 px-4 pointer-events-none">
      <nav 
        className={`pointer-events-auto transition-all duration-500 ease-in-out flex items-center justify-between px-6 py-3 rounded-full border border-white/10 shadow-2xl backdrop-blur-xl bg-[#1B1D23]/80 ${
          isScrolled ? 'w-full max-w-4xl' : 'w-full max-w-6xl'
        }`}
      >
        <button onClick={() => setPage('home')} className="flex items-center gap-2 group shrink-0">
          <div className="w-9 h-9 bg-[#E60000] flex items-center justify-center rounded-full group-hover:rotate-12 transition-all shadow-lg shadow-red-900/20">
            <Dumbbell className="text-white w-5 h-5" />
          </div>
          <span className="font-display text-2xl tracking-tight uppercase text-white">FYRON<span className="text-[#E60000]">FITNESS</span></span>
        </button>
        ...
        <div className="hidden md:flex items-center gap-8 lg:gap-10">
          {navLinks.map((link) => (
            <button 
              ...
              className={`font-display text-lg tracking-[0.1em] uppercase transition-colors ${
                currentPage === link.id ? 'text-[#E60000]' : 'text-[#B0B3BB] hover:text-[#E60000]'
              }`}
            >
              {link.name}
            </button>
          ...
          <button 
            ...
            className="bg-[#E60000] text-white px-6 py-2 rounded-full font-display text-lg tracking-wider transition-all hover:bg-[#FF3333] active:scale-95 shadow-xl shadow-red-900/30"
          >
            JOIN NOW ↗
          </button>
        </div>
        ...
        {isOpen && (
          ...
          >
            {navLinks.map((link) => (
              <button 
                key={link.id} 
                className={`text-left text-2xl font-display uppercase tracking-wider ${currentPage === link.id ? 'text-[#E60000]' : 'text-[#B0B3BB]'}`}
                onClick={() => {
                  setPage(link.id);
                  setIsOpen(false);
                  window.scrollTo({ top: 0, behavior: 'smooth' });
            ...
            <button 
              ...
              className="bg-[#E60000] text-white py-4 rounded-xl font-display text-xl uppercase tracking-widest"
            >
              Join Vizag's Best
            </button>
          </motion.div>
  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden">
      <div className="absolute inset-0 z-0">
        <div className="absolute inset-0 bg-gradient-to-r from-[#1B1D23] via-[#1B1D23]/80 to-transparent z-10" />
      ...
      <div className="max-w-7xl mx-auto px-6 w-full relative z-20 pt-20">
        ...
        >
          <div className="flex items-center gap-4 mb-8">
             <span className="text-[#E60000] font-bold tracking-[0.4em] text-xs uppercase flex items-center gap-2 bg-black/40 backdrop-blur-md py-2 px-4 rounded-full border border-white/10">
          ...
          <h1 className="text-6xl md:text-[140px] font-display leading-[0.8] mb-8 uppercase tracking-tight text-white">
            <span className="text-[#E60000]">FORGE</span> YOUR<br />
          ...
          <div className="flex flex-wrap gap-6">
            <button 
              onClick={() => openModal('join')}
              className="px-12 py-6 bg-[#E60000] hover:bg-[#FF3333] text-white font-display text-xl uppercase tracking-widest rounded transition-all shadow-2xl shadow-red-900/40 active:scale-95"
            >
    ...
    <section id="services" className="py-32 px-6 bg-white">
      <div className="max-w-7xl mx-auto">
        <motion.div 
          ...
          className="mb-16"
        >
          <span className="text-[#E60000] font-bold text-sm tracking-[0.3em] uppercase mb-4 block">World Class Training</span>
          <h2 className="text-6xl font-display uppercase tracking-tight text-black">THE <span className="text-[#E60000]">FYRON</span> EDGE</h2>
        ...
        >
          {categories.map((cat, i) => (
            <motion.div 
              key={i}
              variants={fadeInUp}
              className="bg-gray-50 rounded-lg overflow-hidden group border border-gray-100 hover:border-[#E60000]/30 transition-all duration-500 shadow-xl"
            >
              ...
              <div className="p-8">
                <h3 className="text-3xl font-display uppercase text-black mb-3 group-hover:text-[#E60000] transition-colors">{cat.title}</h3>
                <p className="text-sm text-gray-500 font-bold leading-relaxed mb-6">{cat.description}</p>
                <button className="text-[#E60000] font-display uppercase tracking-widest text-lg flex items-center gap-2 group-hover:gap-4 transition-all">
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-12 gap-16">
          <div className="lg:col-span-5 relative">
            <span className="text-[#E60000] font-bold text-sm tracking-widest uppercase mb-4 block">Built for Excellence</span>
            <h2 className="text-6xl font-display mb-8 leading-none uppercase">VIZAG'S <span className="text-[#E60000]">STRONGEST</span> COMMUNITY</h2>
            <div className="relative aspect-[3/4] rounded-sm overflow-hidden shadow-2xl group">
               ...
               <div className="absolute inset-0 bg-[#E60000]/10 mix-blend-multiply opacity-0 group-hover:opacity-100 transition-opacity" />
            </div>
          </div>
            ...
            <div className="space-y-12">
               <div>
                  <h3 className="text-4xl font-display mb-6 uppercase flex items-center gap-3">
                    Our Philosophy <div className="h-1 w-12 bg-[#E60000]" />
                  </h3>
               ...
               <div className="grid md:grid-cols-2 gap-8">
                  <div className="bg-[#1B1D23] p-8 rounded-lg border-l-4 border-[#E60000]">
    ...
    <section id="bmi" className="py-32 px-6 bg-white overflow-hidden border-y border-gray-100">
       <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-20 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            <span className="text-[#E60000] font-bold text-sm tracking-widest uppercase mb-4 block">Performance Check</span>
            <h2 className="text-6xl font-display text-black uppercase mb-8 leading-tight tracking-tighter">BMI <span className="text-[#E60000]">CALCULATOR</span></h2>
            <div className="space-y-6 max-w-md">
               <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-3">
                     ...
                     <input 
                       ...
                       placeholder="eg: 175" 
                       className="w-full bg-gray-50 border-2 border-gray-100 p-5 rounded-lg focus:ring-2 focus:ring-[#E60000] focus:border-[#E60000] outline-none font-bold text-black placeholder-gray-400 transition-all shadow-sm" 
                     />
                  </div>
                  <div className="space-y-3">
               ...
               <button 
                onClick={calculate}
                className="w-full bg-[#E60000] text-white py-6 font-display text-2xl uppercase tracking-[0.1em] hover:bg-[#FF3333] transition-all shadow-2xl shadow-red-900/40 rounded-lg group flex items-center justify-center gap-3"
               >
            ...
            <AnimatePresence>
              {bmi && (
                <motion.div 
                  ...
                  className="mt-10 p-8 bg-gray-50 text-black rounded-xl shadow-xl border border-[#E60000]/10 relative overflow-hidden group"
                >
                  <div className="flex justify-between items-center relative z-10">
                     <div>
                       <p className="text-xs font-bold text-[#E60000] uppercase mb-2 tracking-[0.2em]">Body Mass Index</p>
                     ...
                     <div className="text-right">
                       ...
                       <p className={`text-4xl font-display uppercase tracking-tighter ${
                         getStatus(bmi) === 'Normal' ? 'text-green-500' : 'text-[#E60000]'
                       }`}>{getStatus(bmi)}</p>
                     </div>
                  </div>
                </motion.div>
    ...
    <section id="pricing" className="py-32 bg-[#1B1D23] relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center mb-24">
           <span className="text-[#E60000] font-bold text-sm tracking-[0.5em] uppercase mb-4 block underline underline-offset-8 decoration-white/10">Access Levels</span>
           <h2 className="text-7xl font-display text-white uppercase mb-6 tracking-tighter">SELECT YOUR <span className="text-[#E60000]">LEVEL</span></h2>
        ...
        <div className="grid md:grid-cols-3 gap-12">
          {plans.map((plan, i) => (
            <motion.div 
              ...
              className={`group p-10 rounded-2xl border-t-8 transition-all duration-500 relative flex flex-col bg-[#23262F] shadow-2xl overflow-hidden ${plan.accent ? 'border-[#E60000] shadow-red-900/30 ring-1 ring-[#E60000]/20' : 'border-white/5 hover:border-[#E60000]/40'}`}
            >
              {/* Background Number */}
              ...
              <div className="relative z-10">
                <div className="mb-8">
                  ...
                  <p className="text-[#E60000] font-bold text-[10px] uppercase tracking-[0.3em]">{plan.tagline}</p>
                </div>
                ...
                <div className="space-y-5 mb-12 flex-grow">
                   ...
                   {plan.features.map((f, j) => (
                     <li key={j} className="flex items-center gap-3 text-gray-300 font-bold text-[11px] uppercase leading-tight list-none">
                       <Check className="text-[#E60000] w-4 h-4 flex-shrink-0" /> {f}
                     </li>
                   ))}
                </div>
                ...
                <button 
                  onClick={() => openModal('join')}
                  className={`w-full py-5 font-display text-xl uppercase tracking-widest rounded transition-all active:scale-95 flex items-center justify-center gap-2 ${plan.accent ? 'bg-[#E60000] text-white hover:bg-[#FF3333] shadow-xl shadow-red-900/40' : 'bg-white/5 text-white hover:bg-white/10 border border-white/10'}`}
                >
  ...
  return (
    <section className="relative h-[60vh] w-full flex items-center justify-center overflow-hidden">
       ...
       <div className="relative z-10 text-center px-6">
          <span className="text-[#E60000] font-bold text-sm tracking-[0.4em] uppercase mb-8 block">Iron Mindset Only</span>
          <h2 className="text-7xl md:text-9xl font-display text-white uppercase mb-12 tracking-tight">FUEL THE <span className="text-[#E60000]">FYRE</span></h2>
          <button className="w-24 h-24 bg-white/10 backdrop-blur-xl rounded-full border-2 border-white/20 flex items-center justify-center hover:scale-110 hover:bg-[#E60000] transition-all group shadow-2xl shadow-red-900/50">
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 mb-24">
          <div className="col-span-1 lg:col-span-1">
            <button onClick={() => window.scrollTo({top: 0, behavior: 'smooth'})} className="flex items-center gap-2 mb-10 group">
              <div className="w-10 h-10 bg-[#E60000] flex items-center justify-center rounded group-hover:rotate-12 transition-all">
                <Dumbbell className="text-white w-6 h-6" />
              </div>
              <span className="font-display text-3xl tracking-tight text-white uppercase">FYRON<span className="text-[#E60000]">FITNESS</span></span>
            </button>
            ...
            <div className="flex gap-4">
              <a href="#" className="w-12 h-12 rounded bg-white/5 flex items-center justify-center hover:bg-[#E60000] transition-all text-white"><Instagram size={22} /></a>
              <a href="#" className="w-12 h-12 rounded bg-white/5 flex items-center justify-center hover:bg-[#E60000] transition-all text-white"><Twitter size={22} /></a>
              <a href="#" className="w-12 h-12 rounded bg-white/5 flex items-center justify-center hover:bg-[#E60000] transition-all text-white"><Facebook size={22} /></a>
            </div>
          </div>

          <div>
            <h4 className="font-display text-2xl mb-10 uppercase text-[#E60000]">Quick Access</h4>
          ...
          <div>
             <h4 className="font-display text-2xl mb-10 uppercase text-[#E60000]">The Hub</h4>
             <ul className="space-y-4 text-gray-500 font-bold text-[12px] uppercase tracking-[0.1em]">
                <li className="flex items-center gap-3"><MapPin size={14} className="text-[#E60000]" /> MVP Colony, Vizag</li>
          ...
          <div>
             ...
             <div className="relative">
                <input type="email" placeholder="YOUR EMAIL" className="w-full bg-[#23262F] border border-white/5 px-6 py-5 rounded-lg focus:outline-none focus:border-[#E60000] text-white text-xs font-bold uppercase tracking-widest shadow-inner" />
                <button className="absolute right-4 top-1/2 -translate-y-1/2 text-[#E60000] hover:text-white transition-colors"><ArrowRight size={22} /></button>
             </div>
          </div>
        </div>
        ...
        <div className="pt-16 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
          ...
          <div className="flex gap-10">
             <span className="text-[10px] font-bold uppercase tracking-widest text-[#E60000]">Performance</span>
             <span className="text-[10px] font-bold uppercase tracking-widest text-white">Discipline</span>
             <span className="text-[10px] font-bold uppercase tracking-widest text-[#E60000]">Result</span>
          </div>
        </div>
      </div>
    </footer>
...
const App = () => {
    ...
    switch(currentPage) {
        ...
        return (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className="pt-40 pb-16 bg-[#23262F] text-center border-b border-white/5">
              <h1 className="text-7xl md:text-9xl font-display text-white uppercase tracking-tighter">OUR <span className="text-[#E60000]">SERVICES</span></h1>
            </div>
            <Services />
            <BMICalculator />
          </motion.div>
      ...
      case 'pricing':
        return (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className="pt-40 pb-16 bg-[#23262F] text-center border-b border-white/5">
              <h1 className="text-7xl md:text-9xl font-display text-white uppercase tracking-tighter">PLAN YOUR <span className="text-[#E60000]">EVOLUTION</span></h1>
            </div>
      ...
      case 'about':
        return (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className="pt-40 pb-16 bg-[#23262F] text-center border-b border-white/5">
              <h1 className="text-7xl md:text-9xl font-display text-white uppercase tracking-tighter">ABOUT <span className="text-[#E60000]">FYRON</span></h1>
            </div>
            <AboutUs />
          </motion.div>
        );
  ...
  return (
    <div className="bg-[#1B1D23] selection:bg-[#E60000] selection:text-white min-h-screen">
